...
class TestMappingExporter(BaseTest):
    def setUp(self):
        self.exporter = MappingExporter()
        ...
        # Create test data
        self.test_data = self.helper.create_test_mapping()
    ...
    def test_export_data(self):
        ...
        expected_file = self.temp_path / f"{MappingFileNames.MAPPING.value}.csv"
        ...
        self.assertEqual(len(result.map), len(self.test_data.map))
        for source_id, target_id in result.map.items():
            self.assertIn(source_id, self.test_data.map)
            self.assertEqual(target_id, self.test_data.map[source_id])

    def test_export_data_file_exists(self):
        # Create test data with different mappings
        first_data = self.helper.create_test_mapping_with_single_entry(
            0
        )  # First test entry
        second_data = self.helper.create_test_mapping_with_single_entry(
            1
        )  # Second test entry
        ...
        copy_path = self.temp_path / f"{MappingFileNames.MAPPING.value}_copy.csv"
        ...
        self.assertEqual(
            result.map[TestConstants.TEST_SOURCE_IDS.value[1]],
            TestConstants.TEST_TARGET_IDS.value[1],
        )

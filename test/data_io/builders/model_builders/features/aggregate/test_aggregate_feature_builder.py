...
class TestAggregateFeatureBuilder(BaseTest):
    ...
    def test_build_valid_data(self):
        ...
        input_group = self.data_helper.create_test_aggregate_feature_hdf5()
        # Test building feature data
        result: AggregateFeatureSetEntry = self.builder.build(input_group)
        ...
        for agg_ix, agg_feature in enumerate(agg_feature_list):
            ...
            for stat_ix, stat in enumerate(agg_feature.descriptive_statistics):
                ...
                self.assertEqual(stat.value, TestConstants.FEATURE_DATA.value[agg_ix][stat_ix])

...
class LinearSvmModel(BaseClassifierModel):
    """Linear-kernel SVM with repeated CV evaluation."""
    ...
    def _build_search(self) -> GridSearchCV:
        ...
        c_grid = [0.1, 1.0, 10.0] if self.fast_mode else [0.01, 0.1, 1.0, 10.0, 100.0]
    ...
    def evaluate(
        ...
        self.final_best_params = {
            "clf__C_mode": max(
                set([params["clf__C"] for params in best_params_history]),
                key=[params["clf__C"] for params in best_params_history].count,
            )
        }

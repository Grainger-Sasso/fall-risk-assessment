...
class UserDataRegistryFileNames(Enum):
    """Enumeration of registry file names."""

    USER_DATA_REGISTRY = "user_data_registry"


class UserDataRegistryImporter(Importer[UserDataRegistry]):
    """Imports registry data from files."""

    def __init__(self):
        reader = CSVFileReader()
        model_builder = UserDataRegistryBuilder()
        file_suffixes = ["csv"]
        super().__init__(reader, model_builder, file_suffixes)

    def import_data(self, directory: Path) -> UserDataRegistry:
        ...
        Returns:
            UserDataRegistry: Imported registry

        Raises:
            FileNotFoundError: If required files not found
        """
        file_paths: Dict[Enum, Path] = self.resolve_file_paths(
            directory, UserDataRegistryFileNames
        )
        ...
        registry_file: CSVFile = self.reader.read(
            file_paths[UserDataRegistryFileNames.USER_DATA_REGISTRY]
        )
